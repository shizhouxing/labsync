...
def get_parser():
    ...
    parser.add_argument('-p', '--prefix', type=str, help='Prefix of model directories')    
    ...
    parser.add_argument('-l', '--loop', type=int, default=1, help='Loop interval')
...
def update(args, ids):
    if not args.server:
        ...
        for id in ids:
            path_remote = '{}{}/log'.format(args.prefix, id)
            logger.info('Updating {}'.format(path_remote))
            tmp = os.path.join(tmp_dir, id)
            if os.path.exists(tmp):
                shutil.rmtree(tmp)
...
def tb_update():
    ...
    if args.prefix:
        logger.warning('Hint: you are able to specify `--prefix` with a `checkpoints` item in the server configuration')
